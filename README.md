...
So inside the folder you just cloned, The first thing we need to create our project
...
On the request `auth/signup.js` file you need to add an `key` tag for the **email**. This is when the user clicks on the `signup` button, you should store the following values in the database:
...
Inside the route, after comparing the confirmation code, you have to set the `status` field of the user to 'Active'. Then return a message  of`confirmation` letting the user know that everything went perfect, or showing the error.

### Iteration 4 - Profile Routes

Finally, you have to create a `profile.js` in folder routes, where you have to return the `username` and the `status` of the user.
